...
/**
 * UAT script (TypeScript) — posts dummy data to /api/sendCV
 * Usage: (from repo root)
 *   npx ts-node scripts/uat/recruitment-uat.ts
 * Environment:
 *   TARGET_URL - full URL to API endpoint (default http://localhost:3000/api/sendCV)
 *   API_KEY - API key for Authorization header (default "RahnWebsite")
 *
 * NOTE: This script will POST JSON to the API endpoint. The server's SMTP settings
 * (EMAIL_HOST, EMAIL_USER, EMAIL_PASS, EMAIL_FROM) control where the mail is sent.
 * By default the repository should be tested with a Mailtrap or similar test SMTP
 * so real addresses are not notified. If you want to send to a real address, set
 * EMAIL_FROM on the server and be careful.
 */
...
// Flags:
// VERBOSE=true or --verbose -> prints detailed debug information
// DRY_RUN=true or --dry-run -> do not actually POST to the server; useful for testing
...
async function run() {
  ...
  // Create a small fake "PDF" (it's plain text but it's fine for UAT)
  const fakePdf = Buffer.from("%PDF-1.4\n% UAT dummy pdf\nHello UAT\n", "utf8").toString("base64");

  const payload = {
    name: dummyName,
    // the `email` field will be used as the destination ONLY when the server
    // environment allows it (ALLOW_TO_OVERRIDE=true). You can set RECIPIENT to
    // override this value for the UAT run.
